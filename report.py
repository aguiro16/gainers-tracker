...
def build_daily_prompt(stats):
    ...
    for s in losing:
        lines.append(f"إشارة #{s['signal_number']}: {s['symbol']} {s['direction']} | خسارة: {s['pnl_pct']}% | R:R: {s['rr']} | الشارت: {s['tradingview_url']}")
    lines.append("\nالمطلوب بالعربية:\n1. الأخطاء\n2. التوصيات")
    return "\n".join(lines)
...
def build_weekly_prompt(stats):
    ...
    for s in closed:
        ...
        if s['pnl_pct']>0: symbol_stats[sym]['wins']+=1
        else: symbol_stats[sym]['losses']+=1
...
إحصائيات الأسبوع:
- إجمالي: {stats['total']} | مغلقة: {len(closed)} | Win Rate: {win_rate}%
